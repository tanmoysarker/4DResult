...
export class S extends Component {
  ...
  async componentDidMount() {
    ...
    if ((time >= 19) && (time <= 20 ) && (weekDay === 0 ||3 ||6) && (today.getMinutes()<=30)){
      this.setState({ 
        tableData5: [],
        tableData6: [],
        tableData7: [],
      ...
      <View>
          <View style={{ paddingTop: 10, paddingHorizontal: 10, flexDirection: 'row', justifyContent: 'space-evenly' }}>
              ...
              <View style={styles.container}>
                <Table style={{ flexDirection: 'row' }} borderStyle={{ borderWidth: 4, borderColor: '#c5cbd6' }}>
                  <TableWrapper style={{ width: 110 }}>
                    <Cell data={(this.state.toggle) === 0 ? "Consolation" : (this.state.toggle) === 1 ? "安慰獎": "Saguhati"} style={{ width: 110, height: 40, backgroundColor: '#CFD7DC' }} textStyle={{ color: '#000', margin: 6, alignSelf: 'center',fontSize:16,fontWeight:'bold' }} />
                    <TableWrapper style={{ flexDirection: 'row',height:550}}>
